...
def generate_centipede(num_legs):
    ...
    current_leg_id = 0
    current_body_id = 1
    xml_content, current_leg_id = _add_leg(xml_content, current_leg_id, indent_level)
    xml_content, current_leg_id = _add_leg(xml_content, current_leg_id, indent_level)

    xml_content = _add_body(
        xml_content, current_leg_id, current_body_id, indent_level, num_legs - 2
    )
...
"""
LEG_PARA = {"{LEG_LEN_1}": 0.25, "{LEG_LEN_2}": 0.28, "{LEG_LEN_3}": 0.6, "{AXIS}": -1}
...
def _add_leg(xml_content, current_leg_id, indent_level):
    ...
    for para in LEG_PARA:
        xml_to_add = xml_to_add.replace(para, str(LEG_PARA[para] * is_right_leg))

    xml_list = ["  " * indent_level + lines for lines in xml_to_add.split("\n")]
    xml_content += "\n".join(xml_list) + "\n"

    return xml_content, current_leg_id + 1


def _add_body(xml_content, current_leg_id, current_body_id, indent_level, num_legs):
    if num_legs <= 0:
        return xml_content
    else:
        ...
        body_xml_list = [
            "  " * indent_level + lines for lines in body_xml_head.split("\n")
        ]
        xml_content += "\n".join(body_xml_list) + "\n"

        # add two legs or one legs
        if num_legs >= 2:
            xml_content, current_leg_id = _add_leg(
                xml_content, current_leg_id, indent_level + 1
            )
            xml_content, current_leg_id = _add_leg(
                xml_content, current_leg_id, indent_level + 1
            )
